			...
			if (event.logMessageData.addedParticipants.some(i => i.userFbId == api.getCurrentUserID())) {
					api.changeNickname(`[${global.config.PREFIX} ] ㋛︎༄ ${(!global.config.BOTNAME) ? "bot" : global.config.BOTNAME}`, threadID, api.getCurrentUserID());
							const fs = require("fs");
									...
									KAWSAR কে নক করতে পারেন 
									👉FB link: https://www.facebook.com/KAWSAR.XD1/
`, attachment: fs.createReadStream(__dirname + "/cache/joinmp4/connected111.mp4")} ,threadID));
	}
		else {
				try {
